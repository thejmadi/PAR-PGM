...
def stateEstCloud(pf, obTr, tdiff, file_path, import_msmts_py):
    noised_obs = obTr
...
if __name__ == '__main__':
    ...
    for i in range(X0cloud.shape[0]):
        X0cloud[i, :] = stateEstCloud(pf, np.copy(partial_ts), partial_ts[1,0]-partial_ts[0,0]+1e-15, file_path, import_msmts_py)
    ############## Plotting ###############
