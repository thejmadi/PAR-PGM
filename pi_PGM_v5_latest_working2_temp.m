...
for mc_idx = 4:4
    ...
    num_IOD_particles = 10000;
    Lp = 8000;
    % Num of observers
    total_num_agents = 2;
    num_agents = 0;
    agent_is_active = false(total_num_agents, 1);
    num_msmt_for_IOD = [15; 15];
    ts_to_perform_IOD = [0; 0];
    num_clouds_per_agent = [1; 1];
    ...
    for ts = 1:num_timesteps-1
        ...
        %% IOD
        for ob = 1:total_num_agents
            if ((sum(combined_msmt_data(1:ts, 2, ob)) >= num_msmt_for_IOD(ob)) && (~agent_is_active(ob)))
                nfit = 4;
                X0cloud_temp = zeros(num_IOD_particles, 6);
                for i = 1:num_IOD_particles
                    X0cloud_temp(i,:) = stateEstCloud(num_msmt_for_IOD(ob), ts, nfit, theta_f, R_f{ob}, combined_msmt_data(:, :, ob), low_lim, up_lim);
                end
                % TODO: Check whether X0cloud clears correctly
                X0cloud = enforceCislunarBounds(X0cloud_temp, t_prev, obs_lat{ob}, obs_lon{ob}, dist2km, vel2kms, low_lim, up_lim, vel_lim);%X0cloud(1:j,:);
                plotStateSpace(X0cloud, ...
                                ...
                                vel2kms, ...
                                colors, ...
                                sprintf('Timestep: %3.4f Hours Obs: %i', [t_prev*time2hr, ob]), ...
                                sprintf('%s/Observer%i/iodCloud.png', save_loc, ob))
        ...
        if (sum(agent_is_active) >= 2) % If 2 or more agents have completed IOD
            %% Fusion
            if(fuse_orig_clouds(fusion_idx) == true && ts == time_of_fusion(fusion_idx))
                disp("Fusing Clouds")
                % Convert clouds to Synodic Frame
                converted_cloud_1 = backConvertSynodic(Xp_cloudp{1, 1}, t_prev, obs_lat{1}, obs_lon{1});
                converted_cloud_2 = backConvertSynodic(Xp_cloudp{2, 1}, t_prev, obs_lat{2}, obs_lon{2});
                
                % Fuse Clouds
                [p3_simple, ~, weight_update_p3_1, weight_update_p3_2, fusion_bin_edges] = fusionMethods(converted_cloud_1, converted_cloud_2, cluster_by, Kmax, 2, Lp, disp_diagnostics, save_loc, dist2km, vel2kms, fusion_idx);
        
                % Convert clouds back to Topo Frame
                Xp_cloudp{1, num_clouds_per_agent(1)+1} = convertToTopo(p3_simple, t_prev, obs_lat{1}, obs_lon{1});
                Xp_cloudp{1, num_clouds_per_agent(1)+2} = convertToTopo(weight_update_p3_1, t_prev, obs_lat{1}, obs_lon{1});
                Xp_cloudp{2, num_clouds_per_agent(2)+1} = convertToTopo(p3_simple, t_prev, obs_lat{2}, obs_lon{2});
                Xp_cloudp{2, num_clouds_per_agent(2)+2} = convertToTopo(weight_update_p3_2, t_prev, obs_lat{2}, obs_lon{2});
                cloud_names = [cloud_names, sprintf("%s %3.1f", fusion_types(2), t_prev*time2hr), sprintf("%s %3.1f", fusion_types(3), t_prev*time2hr)];
                
                fuse_orig_clouds(fusion_idx) = false;
                num_clouds_per_agent(1) = num_clouds_per_agent(1) + num_new_clouds_per_agent;
                num_clouds_per_agent(2) = num_clouds_per_agent(2) + num_new_clouds_per_agent;
                ...
                for ob = 1:total_num_agents
                    if ((size(Xp_cloudp, 2) > 1) && (plot_comb_clouds))
                        ...
                        plotStateSpaceCombined(plotting_cloud, ...
                                                ...
                                                colors, ...
                                                cloud_names, ...
                                                sprintf('Timestep: %3.4f Hours Fusion Ob: %i', [t_prev*time2hr, ob]), ...
                        ...
                        plotMsmtSpaceCombined(plotting_cloud, ...
                                                ...
                                                colors, ...
                                                cloud_names, ...
                                                sprintf('Az-El Timestep: %3.4f Hours Fusion Ob: %i', [t_prev*time2hr, ob]), ...
                                                sprintf('%s/Observer%i/Topo/Combined/Timestep_%i_0C_combined.png', save_loc, ob, ts), ...
                                                false)
        
                        plotting_cloud = cell(num_clouds_per_agent(ob));
                        for cloud = 1:num_clouds_per_agent(ob)
                            plotting_cloud{cloud} = Topo2ECI(Xp_cloudp{ob, cloud}, t_prev, obs_lat{ob}, obs_lon{ob});
                        end
                        plotting_truth = Topo2ECI(Xprop_truth{ob}, t_prev, obs_lat{ob}, obs_lon{ob});
                        plotStateSpaceCombined(plotting_cloud, ...
                                                plotting_truth, ...
                                                num_clouds_per_agent(ob), ...
                                                dist2km, ...
                        ...
                        plotting_cloud = cell(num_clouds_per_agent(ob));
                        for cloud = 1:num_clouds_per_agent(ob)
                            plotting_cloud{cloud} = backConvertSynodic(Xp_cloudp{ob, cloud}, t_prev, obs_lat{ob}, obs_lon{ob});
                        end
                        plotting_truth = backConvertSynodic(Xprop_truth{ob}, t_prev, obs_lat{ob}, obs_lon{ob});
                        plotStateSpaceCombined(plotting_cloud, ...
                                                plotting_truth, ...
                                                num_clouds_per_agent(ob), ...
                                                dist2km, ...
        ...
        for ob=1:total_num_agents
            if agent_is_active(ob) == true
                for cloud = 1:num_clouds_per_agent(ob)
                    ...
                    % Propagation Step
                    Xm_cloud_tmp = propagate(Xp_cloudp{ob, cloud}, t_prev, interval, obs_lat{ob}, obs_lon{ob});
                    Xm_cloud{ob, cloud} = enforceCislunarBounds(Xm_cloud_tmp, t_prior, obs_lat{ob}, obs_lon{ob}, dist2km, vel2kms, low_lim, up_lim, vel_lim);
                    %end
                end
            end
            ...
            if (agent_is_active(ob) == true)
                if (combined_msmt_data(ts+1, 2, ob) == true)
                    ...
                    for cloud = 1:num_clouds_per_agent(ob)
                        % Update Step
                        for k = 1:K{ob, cloud}
                            [mu_p{1, cloud, k}, P_p{1, cloud, k}] = kalmanUpdate(zt{ob}, cPoints{1, cloud, k}, R_vv, mu_c{1, cloud, k}, P_c{1, cloud, k}, h);
                            P_p{1, cloud, k} = (P_p{1, cloud, k} + P_p{1, cloud, k}')/2;
                        end
                        % Weight update
                        wp{1, cloud} = weightUpdate(wm{1, cloud}, Xm_cloud{ob, cloud}, idx{ob, cloud}, zt{ob}, R_vv, h);
                    end
                    ...
                    for cloud = 1:num_clouds_per_agent(ob)
                        ...
                        for i = 1:Lp
                            [Xp_cloudp_temp(i,:), c_id_temp(i)] = drawFrom(wp{1, cloud}, mu_p(1, cloud, :), P_p(1, cloud, :)); 
                        end
                    ...
                    for cloud = 1:num_clouds_per_agent(ob)
                        ...
                        wp{1, cloud} = [1];
                        mu_p{1, cloud, 1} = mean(Xp_cloud{ob, cloud});
                        P_p{1, cloud, 1} = cov(Xp_cloud{ob, cloud});
                    end
        ...
        for ob = 1:total_num_agents
            ...
            if (msmt_exists == true && agent_is_active(ob) == true)
                for cloud = 1:num_clouds_per_agent(ob)
                    if (plot_indv_clouds)
                        ...
                        plotStateSpace(plotting_cloud, ...
                                        ...
                                        vel2kms, ...
                                        colors, ...
                                        sprintf('Timestep: %3.4f Hours (Prior) Ob: %i', [t_prior*time2hr, ob]), ...
                        ...
                        plotMsmtSpace(plotting_cloud, ...
                                        ...
                                        idx{ob, cloud}, ...
                                        colors, ...
                                        sprintf('Az-El Timestep: %3.4f Hours (Prior) Ob: %i', [t_prior*time2hr, ob]), ...
                                        sprintf('%s/Observer%i/Topo/Timestep_%i_1C_cloud_%i.png', save_loc, ob, ts+1, cloud), ...
                                        msmt_exists)
            
                        plotting_cloud = Topo2ECI(Xm_cloud{ob, cloud}, t_prior, obs_lat{ob}, obs_lon{ob});
                        plotting_truth = Topo2ECI(Xprop_truth{ob}, t_prior, obs_lat{ob}, obs_lon{ob});
                        plotStateSpace(plotting_cloud, ...
                                        plotting_truth, ...
                                        K{ob, cloud}, ...
                                        idx{ob, cloud}, ...
                        ...
                        plotting_cloud = backConvertSynodic(Xm_cloud{ob, cloud}, t_prior, obs_lat{ob}, obs_lon{ob});
                        plotting_truth = backConvertSynodic(Xprop_truth{ob}, t_prior, obs_lat{ob}, obs_lon{ob});
                        plotStateSpace(plotting_cloud, ...
                                        plotting_truth, ...
                                        K{ob, cloud}, ...
                                        idx{ob, cloud}, ...
                ...
                if ((size(Xm_cloud, 2) > 1) && (plot_comb_clouds))
                    plotting_cloud = cell(num_clouds_per_agent(ob));
                    for cloud = 1:num_clouds_per_agent(ob)
                    ...
                    plotting_cloud = cell(num_clouds_per_agent(ob));
                    for cloud = 1:num_clouds_per_agent(ob)
                        plotting_cloud{cloud} = Topo2ECI(Xm_cloud{ob, cloud}, t_prior, obs_lat{ob}, obs_lon{ob});
                    end
                    plotting_truth = Topo2ECI(Xprop_truth{ob}, t_prior, obs_lat{ob}, obs_lon{ob});
                    plotStateSpaceCombined(plotting_cloud, ...
                                            plotting_truth, ...
                    ...
                    plotting_cloud = cell(num_clouds_per_agent(ob));
                    for cloud = 1:num_clouds_per_agent(ob)
                        plotting_cloud{cloud} = backConvertSynodic(Xm_cloud{ob, cloud}, t_prior, obs_lat{ob}, obs_lon{ob});
                    end
                    plotting_truth = backConvertSynodic(Xprop_truth{ob}, t_prior, obs_lat{ob}, obs_lon{ob});
                    plotStateSpaceCombined(plotting_cloud, ...
                                            plotting_truth, ...
        ...
        for ob = 1:total_num_agents
            ...
            if agent_is_active(ob) == true
                for cloud = 1:num_clouds_per_agent(ob)
                    ...
                    if (plot_indv_clouds)
                        ...
                        plotMsmtSpace(plotting_cloud, ...
                                        ...
                                        c_id{ob, cloud}, ...
                                        colors, ...
                                        sprintf('Az-El Timestep: %3.4f Hours (Posterior) Ob: %i', [t_prior*time2hr, ob]), ...
                                        sprintf('%s/Observer%i/Topo/Timestep_%i_2C_cloud_%i.png', save_loc, ob, ts+1, cloud), ...
                                        msmt_exists)
        
                        plotting_cloud = Topo2ECI(Xp_cloudp{ob, cloud}, t_prior, obs_lat{ob}, obs_lon{ob});
                        plotting_truth = Topo2ECI(Xprop_truth{ob}, t_prior, obs_lat{ob}, obs_lon{ob});
                        plotStateSpace(plotting_cloud, ...
                                        plotting_truth, ...
                                        K{ob, cloud}, ...
                        ...
                        plotting_cloud = backConvertSynodic(Xp_cloudp{ob, cloud}, t_prior, obs_lat{ob}, obs_lon{ob});
                        plotting_truth = backConvertSynodic(Xprop_truth{ob}, t_prior, obs_lat{ob}, obs_lon{ob});
                        plotStateSpace(plotting_cloud, ...
                                        plotting_truth, ...
                                        K{ob, cloud}, ...
                ...
                if (plot_comb_clouds)
                    ...
                    plotting_cloud = cell(num_clouds_per_agent(ob));
                    for cloud = 1:num_clouds_per_agent(ob)
                        plotting_cloud{cloud} = Topo2ECI(Xp_cloudp{ob, cloud}, t_prior, obs_lat{ob}, obs_lon{ob});
                    end
                    plotting_truth = Topo2ECI(Xprop_truth{ob}, t_prior, obs_lat{ob}, obs_lon{ob});
                    plotStateSpaceCombined(plotting_cloud, ...
                                            plotting_truth, ...
                    ...
                    plotting_cloud = cell(num_clouds_per_agent(ob));
                    for cloud = 1:num_clouds_per_agent(ob)
                        plotting_cloud{cloud} = backConvertSynodic(Xp_cloudp{ob, cloud}, t_prior, obs_lat{ob}, obs_lon{ob});
                    end
                    plotting_truth = backConvertSynodic(Xprop_truth{ob}, t_prior, obs_lat{ob}, obs_lon{ob});
                    plotStateSpaceCombined(plotting_cloud, ...
                                            plotting_truth, ...
    ...
    for ob = 1:total_num_agents
        ...
        for cloud = 1:num_clouds_per_agent(ob)
            ...
            sg = sprintf('Timestamp: %1.5f Ob: %i', [t_prior*time2hr, ob]);
            sgtitle(sg)
...
function [Xfit] = stateEstCloud(num_msmt_for_IOD, ts, nfit, theta_f, R_f, combined_msmt_data, low_lim, up_lim)
    ...
    noised_obs = [combined_msmt_data(msmt_existance_mask, 1), reshape(data_vec, 3, []).'];
    noised_obs2 = noised_obs;
    ...
    hdR_p = hdR(ts - num_msmt_for_IOD+1:ts,:); % Matrix for a partial half-day observation
...
function [p3_resampled, p3_tallest_peaks, weight_update_p3_1, weight_update_p3_2, bin_axes]  = fusionMethods(p1, p2, cluster_by, Kmax, num_agents, num_particles, display_diagnostics, save_loc, dist2km, vel2kms, fusion_idx)
    bin_axes = cell(1, 3);
    p3 = vertcat(p1, p2);
    %{
