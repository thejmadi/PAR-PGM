...
parfor i = 1:length(X0cloud(:,1))
    X0cloud(i,:) = stateEstCloud(pf, nfit, theta_f, R_f, partial_ts, (partial_ts(2,1) - partial_ts(1,1)) + 1e-15);
end
...
for ts = idx_start:(idx_end-1)
    ...
    if(idx_meas ~= 0)  
        ...
        K = 1;
        mu_p = cell(1, 1); mu_c = mu_p;
        P_p = cell(1, 1); P_c = P_p;
        wm = [1];
        cPoints = cell(1, 1); 

        Xp_cloud = Xm_cloud; cPoints{1} = Xp_cloud;
        wp = [1];
        mu_p{1} = mean(Xp_cloud)'; mu_c{1} = mu_p{1}';
        P_p{1} = cov(Xp_cloud); P_c{1} = P_p{1};
    end
...
function [Xfit] = stateEstCloud(pf, nfit, theta_f, R_f, obTr, tdiff)
    noised_obs = obTr;
