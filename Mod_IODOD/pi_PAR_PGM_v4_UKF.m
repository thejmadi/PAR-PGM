...
load("X0cloudComp2.mat", "X0cloudComp");
X0cloud = X0cloudComp;
...
mu_0 = mean(X0cloud,1); mu_0 = reshape(mu_0, [1 length(mu_0)]);
P_0 = cov(X0cloud);
...
sigs(1,:) = reshape(mu_0, [length(mu_0) 1]);
for i = 1:n
    sigs(i+1,:) = (mu_0 + sqrt(n + lambda) * S(:,i))';
    sigs(i+1+n,:) = (mu_0 - sqrt(n + lambda) * S(:,i))';
end
...
function [mu_p, P_p] = ukfUpdate(zk, R, mu_m, P_m, h)
    ...
    mzk = zeros(1,length(zk));
    for i = 1:(2*n+1)
        zetas(i,:) = h(sigs(i,:));
        mzk = mzk + zetas(i,:) * wm(i);
    end
