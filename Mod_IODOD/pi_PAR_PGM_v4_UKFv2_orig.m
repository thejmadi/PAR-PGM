...
% Modify intFerpolation interval length such that you are pieceing through
% enough points.
if (in_len < nfit + 1)
    in_len = nfit + 1;
...
% X0cloudComp = X0cloud; save("X0cloudComp2.mat", "X0cloudComp");
...
% For the first timestep, take the particle sample
mu_p{K} = mean(X0cloud, 1);
P_p{K} = cov(X0cloud); 

% Xprop_truth = reshape(Xprop_truth, [1 length(Xprop_truth)]); % Convert to row vector
Xprop_truth = propagate(Xot_truth', t_int, interval);
[Xsigs, wc, wmean] = ukfProp(t_int, interval, mu_p{K}, P_p{K});

mu_c{K} = wmean*Xsigs;
P_c{K} = zeros(length(mu_c{K}));

for j = 1:2*length(Xsigs(1,:))+1
    P_c{K} = P_c{K} + wc(j) * (Xsigs(j,:)' - mu_c{K}') * (Xsigs(j,:)' - mu_c{K}')';
end
...
if (idx_meas ~= 0) % i.e. there exists a measurement
    ...
    for i = 1:K 
        [mu_p{i}, P_p{i}] = ukfUpdate(zt, R_vv, Xsigs, wc, wmean, h);
        % [mu_p{i}, P_p{i}] = kalmanUpdate(zt, cPoints{i}, R_vv, mu_c{i}, P_c{i}, h);
    end

    % Weight update
    idx = ones(length(Xm_cloud),1);
    wp = wm;
...
t_end = cTimes(1); % First observation of new pass + one more time step
...
mxstore(1,:) = mean(X0cloud, 1); mxstore(2,:) = mean(Xm_cloud, 1); mxstore(3,:) = mean(Xp_cloud, 1);
exstore(1,:) = mean(X0cloud, 1) - Xot_truth'; exstore(2,:) = mean(Xm_cloud, 1) - Xprop_truth; exstore(3,:) = mean(Xp_cloud, 1) - Xprop_truth;
sxstore(1,:) = sqrt(diag(cov(X0cloud))); sxstore(2,:) = sqrt(diag(cov(Xm_cloud))); sxstore(3,:) = sqrt(diag(cov(Xp_cloud)));
...
for ts = idx_start:(idx_end-1)
    ...
    [Xsigs, wc, wmean] = ukfProp(to, interval, mu_p{K}, P_p{K});
    mu_c{K} = wmean*Xsigs;
    
    P_c{K} = zeros(length(mu_c{K}));
    
    for i = 1:2*length(Xsigs(1,:))+1
        P_c{K} = P_c{K} + wc(i) * (Xsigs(i,:)' - mu_c{K}') * (Xsigs(i,:)' - mu_c{K}')';
    end
    ...
    if(idx_meas ~= 0)  
        ...
        Xp_cloudp = Xm_cloud; cPoints{1} = Xp_cloudp;
        wp = [1];
        mu_p{1} = mu_c{1};
        P_p{1} = P_c{1};
    end
    ...
    mxstore(2*tau+2, :) = mu_c{1}; mxstore(2*tau+2, :) = mu_p{1};
...
function [prop_sigs, wc, wm] = ukfProp(t_int, interval, mu_p, P_p)
...
function [mu_p, P_p] = ukfUpdate(zk, R, sigs, wc, wm, h)
    ...
    P_m = zeros(n);
    for i = 1:(2*n+1)
        P_m = P_m + wm(i) * (sigs(i,:)' - mu_m') * (sigs(i,:)' - mu_m')';
    end
