...
for k = 1:Nt
    scatter(dist2km*Xm_cloud{k}(:,1), dist2km*Xm_cloud{k}(:,2), 'filled', ...
        'HandleVisibility', 'off', 'MarkerFaceColor', colors(Kn*(k-1)+k));
    hold on;
end
for j = 1:Nt
...
for k = 1:K
    ...
    mu_pExp(k,:) = mu_p{k};
    scatter3(dist2km*clusterPoints(:,1), dist2km*clusterPoints(:,2), dist2km*clusterPoints(:,3), 'filled', 'MarkerFaceColor', 201
k));
    hold on;
end
...
for ts = idx_start:(idx_end-1) 
    ...
    for b = 1:Nt
        ...
        if(idx_meas ~= 0)  
            ...
            if (b == 1)
                wm = zeros(K,Nt); wp = wm;
            elseif(K > length(wm(:,b-1)))
                wm_tmp = zeros(K,Nt); wp_tmp = wm_tmp;
                wm_tmp(1:length(wm(:,b-1)),b-1) = wm(:,b-1);
                wm = wm_tmp; wp = wp_tmp;
            end
    ...
    % if(1) % Use for all time steps
        ...
        if (mnum > 0)
            ...
            for j = 1:Nt
                for k = 1:K
                    Zmcloud = zeros(length(cPoints{k,j}(:,1)), length(zt(:,j)));
