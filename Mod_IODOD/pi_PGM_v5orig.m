...
R_f = 0.30; % Range percentage error covariance
...
pf = 3/length(hdR(:,1)); % A factor between 0 to 1 describing the length of the day to interpolate [x, y]
nfit = 2; % Order of polynomial fitting (typically around 3-4)
...
parfor i = 1:length(X0cloud_all(:,1))
    X0cloud_all(i,:) = stateEstCloud(pf, nfit, theta_f, R_f, partial_ts, (partial_ts(2,1) - partial_ts(1,1)) + 1e-15);
end
...
for i = 1:length(X0cloud_all(:,1))
    if(norm(X0cloud_all(i,1:3) + reo) > 36000/dist2km && ...
            norm(X0cloud_all(i,1:3) + reo) <= 550000/dist2km) % Upper bound according to The Aerospace Corporation
        j = j + 1;
...
for i = 1:length(Xm_cloud(:,1))
    if(norm(Xm_cloud(i,1:3) + reo) > 36000/dist2km && ...
            norm(Xm_cloud(i,1:3) + reo) <= 550000/dist2km)
        j = j + 1;
...
colors = ["Red", "Blue", "Green", "Yellow", "Magenta", "Cyan", "#500000", "#bf5700", "#00274c", "Black"];
...
function [Xfit] = stateEstCloud(pf, order_fit, theta_f, R_f, obTr, tdiff)
    noised_obs = obTr;
