...
% x0 = [1.16429257222878	-0.0144369085836121	0	-0.0389308426824481 0.0153488211249537	0]; % L2 Lagrange Point Approach (after 48 hours)
% x0 = [1.15682731737570	-0.00655111085719336	0	-0.0136330716779653	0.0364901829970001	0]; % L2 Lagrange Point Approach (after 18 hours)
...
% x0 = [1.0754, 0, 0.2022, 0 -0.1926, 0]; % 3:1 Halo Orbit (from Grebow MS Thesis 2006)
% x0 = [1.04395323521727	0.0918424086357342	0.138550032791493	0.101047496456487	-0.0901461331399529	0.225023512520260]; % 3:1 Halo Orbit (from above) minus 60 hours
...
shutStart = end_t;
% end_t = 18/time2hr - tstamp1; tstamp = end_t + 1e-10;
...
%% Longer-term scheduling (Part 1)
%{
tstamp = t(end); % Begin new trajectory where we left off
end_t = (31*24)/time2hr; % 26-day sensor shutoff time
interval = 16/time2hr; % Only one propagation step
tspan = tstamp:interval:(end_t+1e-11); % Schedule to take measurements once every 8 hours
x0_tmp = dx_dt(end,:); t(end) = []; dx_dt(end,:) = []; shutStart = tstamp;
...
for i = 1:length(t)
    if (norm(cross(rot_topo(i,:), rom_topo(i,:)))/norm(rot_topo(i,:)) > Rm ...
            && (t(i) <= shutStart || t(i) > (28*24)/time2hr)) % Not behind moon and sensor not shut off
        j = j + 1;
        t_valid(j,1) = t(i);
