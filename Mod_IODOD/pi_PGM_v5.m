...
largest_diff = noised_obs(2,1) - noised_obs(1,1);
for j = 2:length(noised_obs(:,1))
    if (noised_obs(j,1) - noised_obs(j-1,1) > largest_diff+1e-11)
        largest_diff = noised_obs(j,1) - noised_obs(j-1,1);
        idx_cVal = j-1;
    end
end
cVal = noised_obs(idx_cVal,1);
...
X0cloud = zeros(size(X0cloud_all)); j = 0;
reo = getObserverPos(t_truth); reo = reshape(reo, [1 length(reo)]);

for i = 1:length(X0cloud_all(:,1))
    if(norm(X0cloud_all(i,1:3) + reo)*dist2km > low_lim && ...
            norm(X0cloud_all(i,1:3) + reo)*dist2km <= up_lim && ... % Upper bound according to The Aerospace Corporation
            norm(X0cloud_all(i,4:6))*vel2kms <= vel_lim) % Escape velocity of the solar system
        j = j + 1;
        X0cloud(j,:) = X0cloud_all(i,:);
    end
end
X0cloud = X0cloud(1:j,:);

% load("X0cloudComp2.mat", "X0cloudComp"); X0cloud = X0cloudComp;
% X0cloud = X0cloud_all;
...
% X0cloudComp = X0cloud; 
% save("X0cloudComp.mat", "X0cloudComp");
...
Xm_cloud = X0cloud; t_int = t_truth;
...
% Cull out non-cislunar estimates
Xm_cloud_tmp = zeros(size(Xm_cloud)); j = 0;
reo = getObserverPos(t_int); reo = reshape(reo, [1 length(reo)]);

for i = 1:length(Xm_cloud(:,1))
    if(norm(Xm_cloud(i,1:3) + reo)*dist2km > low_lim && ...
            norm(Xm_cloud(i,1:3) + reo)*dist2km <= up_lim)
        j = j + 1;
        Xm_cloud_tmp(j,:) = Xm_cloud(i,:);
    end
end
Xm_cloud = Xm_cloud_tmp(1:j,:);
...
% normalization.
rc = dist2km*Xm_cloud(:,1:3);
vc = vel2kms*Xm_cloud(:,4:6);
...
Zm_cloud = zeros(length(Xm_cloud(:,1)), length(noised_obs(1,3:4)));

for i = 1:length(Xm_cloud(:,1))
    Zm_cloud(i,:) = h(Xm_cloud(i,:));
end

% Cluster using K-means clustering algorithm
% [idx, ~] = kmeans(Zm_cloud, K); 
[idx, ~] = kmeans(Xm_norm, K); % Cluster just on position and velocity; Normalize the whole thing
colors = ["Red", "Blue", "Green", "Yellow", "Magenta", "Cyan", "#500000", "#bf5700", "#00274c", "#8c2033", "#b69836", "#57fd91"];
contourCols = lines(max(Kn,Kmax));
...
Lp = 4000;
Xp_cloud = zeros(Lp, length(Xprop_truth));
c_id = zeros(Lp,1);
parfor i = 1:L
...
% Find and set tche start and end times to simulation
[idx_meas, c_meas] = find(abs(hdR(:,1) - tpr) < 1e-10);
interval = hdR(idx_meas,c_meas) - hdR(idx_meas-1,c_meas);

[idx_crit, ~] = find(abs(full_ts(:,1)) >= (7.5*24)/time2hr, 1, 'first'); % Find the index of the last time step before a certain number of days have passed since orbit propagation
...
txstore = zeros(2*l_filt-1, 1); % Time storage
mxstore = zeros(2*l_filt-1, length(mu_c{1})); % Mean storage
exstore = zeros(2*l_filt-1, length(mu_c{1})); % SEE storage
sxstore = zeros(2*l_filt-1, length(mu_c{1})); % Standard deviation storage

txstore(1) = full_ts(idx_prop,1); txstore(2) = full_ts(idx_prop+1,1); txstore(3) = full_ts(idx_prop+1,1);
mxstore(1,:) = mean(X0cloud, 1); 
exstore(1,:) = mean(X0cloud, 1) - Xot_truth'; % 
sxstore(1,:) = sqrt(diag(cov(X0cloud))); % 

mcsum = zeros(size(mu_c{1})); Pcsum = zeros(size(P_c{1})); mpsum = mcsum; Ppsum = Pcsum;
for k = 1:K
    mcsum = mcsum + wm(k)*mu_c{k}; mpsum = mpsum + wp(k)*mu_p{k}';
end
for k = 1:K
    Pcsum = Pcsum + wm(k) * (P_c{k} + (mu_c{k} - mcsum)' * (mu_c{k} - mcsum)); Ppsum = Ppsum + wp(k) * (P_p{k} + (mu_p{k}' - mpsum)' * (mu_p{k}' - mpsum));
end

mxstore(2,:) = mcsum; mxstore(3,:) = mpsum;
sxstore(2,:) = sqrt(diag(Pcsum)); sxstore(3,:) = sqrt(diag(Ppsum));
exstore(2,:) = mcsum - Xprop_truth; exstore(3,:) = mpsum - Xprop_truth;
...
for ts = idx_start:(idx_end-1)
    ...
    % Cull out non-cislunar estimates
    Xm_cloud_tmp = Xm_cloud; j = 0;
    reo = getObserverPos(tpr); reo = reshape(reo, [1 length(reo)]);
    ...
    if(idx_meas ~= 0)  
        ...
        % if(any(abs(tpr - cTimes) < 1e-10))
            ...
            hold on;
            for k = 1:K
                if(~anynan(contours_cell{k}))
                    contour(dist2km*X1, dist2km*X2, dist2km*Z_cell{k}, dist2km*real(contours_cell{k}), 'LineWidth', 2, 'LineColor', contourCols(k,:));
                end
            end    
            ...
            for k = 1:K
                if(~anynan(contours_cell{k}))
                    contour(vel2kms*X1, dist2km*X2, vel2kms*Z_cell{k}, vel2kms*real(contours_cell{k}), 'LineWidth', 2, 'LineColor', contourCols(k,:));
                end
            end    
            % Overlay scatter points
        ...
        txstore(2*tau+2) = tpr; txstore(2*tau+3) = tpr;
        mxstore(2*tau+2,:) = mcsum; mxstore(2*tau+3,:) = mpsum;
        sxstore(2*tau+2,:) = sqrt(diag(Pcsum)); sxstore(2*tau+3,:) = sqrt(diag(Ppsum));
        exstore(2*tau+2,:) = mcsum - Xprop_truth; exstore(2*tau+3,:) = mpsum - Xprop_truth;
        ...
        txstore(2*tau+2) = tpr; txstore(2*tau+3) = tpr;
        mxstore(2*tau+2,:) = mu_c{1}; mxstore(2*tau+3,:) = mu_p{1};
        sxstore(2*tau+2,:) = sqrt(diag(P_c{1})); sxstore(2*tau+3,:) = sqrt(diag(P_p{1}));
        exstore(2*tau+2,:) = mu_c{1} - Xprop_truth; exstore(2*tau+3,:) = mu_p{1} - Xprop_truth;
    end

    if(abs(tpr - cVal) < 1e-11)
        XtopoPreShutoff = Xp_cloud;
        up_lim = 550000;
        save("XtopoPreShutoff_92.mat", "XtopoPreShutoff");
    end
    ...
    % if(any(abs(tpr - cTimes) < 1e-10))
        ...
        for k = 1:K
            if(~anynan(contours_cell{k}))
                contour(vel2kms*X1, dist2km*X2, vel2kms*Z_cell{k}, vel2kms*contours_cell{k}, 'LineWidth', 2, 'LineColor', contourCols(k,:));
            end
        end 
        % Overlay scatter points
...
subplot(2,3,1)
plot(time2hr*txstore, dist2km*exstore(:,1), 'r-')
hold on;
plot(time2hr*txstore, -3*dist2km*sxstore(:,1), 'b-')
hold on;
plot(time2hr*txstore, 3*dist2km*sxstore(:,1), 'b-')
...
subplot(2,3,2)
plot(time2hr*txstore, dist2km*exstore(:,2), 'r-')
hold on;
plot(time2hr*txstore, -3*dist2km*sxstore(:,2), 'b-')
hold on;
plot(time2hr*txstore, 3*dist2km*sxstore(:,2), 'b-')
...
subplot(2,3,3)
plot(time2hr*txstore, dist2km*exstore(:,3), 'r-')
hold on;
plot(time2hr*txstore, -3*dist2km*sxstore(:,3), 'b-')
hold on;
plot(time2hr*txstore, 3*dist2km*sxstore(:,3), 'b-')
...
subplot(2,3,4)
plot(time2hr*txstore, vel2kms*exstore(:,4), 'r-')
hold on;
plot(time2hr*txstore, -3*vel2kms*sxstore(:,4), 'b-')
hold on;
plot(time2hr*txstore, 3*vel2kms*sxstore(:,4), 'b-')
...
subplot(2,3,5)
plot(time2hr*txstore, vel2kms*exstore(:,5), 'r-')
hold on;
plot(time2hr*txstore, -3*vel2kms*sxstore(:,5), 'b-')
hold on;
plot(time2hr*txstore, 3*vel2kms*sxstore(:,5), 'b-')
...
subplot(2,3,6)
plot(time2hr*txstore, vel2kms*exstore(:,6), 'r-')
hold on;
plot(time2hr*txstore, -3*vel2kms*sxstore(:,6), 'b-')
hold on;
plot(time2hr*txstore, 3*vel2kms*sxstore(:,6), 'b-')
...
savefig(gcf, './Simulations/sees.fig');
saveas(gcf, './Simulations/sees.png', 'png');

%%
figure(12)

subplot(2,3,1)
plot(time2hr*txstore, 100*exstore(:,1)./sxstore(:,1))
xlabel('Time (hr.)')
ylabel(sprintf('Percentage Error Relative to 1\\sigma'), 'Interpreter', 'tex')
title('X')


subplot(2,3,2)
plot(time2hr*txstore, 100*exstore(:,2)./sxstore(:,2))
xlabel('Time (hr.)')
ylabel(sprintf('Percentage Error Relative to 1\\sigma'), 'Interpreter', 'tex')
title('Y')

subplot(2,3,3)
plot(time2hr*txstore, 100*exstore(:,3)./sxstore(:,3))
xlabel('Time (hr.)')
ylabel(sprintf('Percentage Error Relative to 1\\sigma'), 'Interpreter', 'tex')
title('Z')

subplot(2,3,4)
plot(time2hr*txstore, 100*exstore(:,4)./sxstore(:,4))
xlabel('Time (hr.)')
ylabel(sprintf('Percentage Error Relative to 1\\sigma'), 'Interpreter', 'tex')
title('Xdot')

subplot(2,3,5)
plot(time2hr*txstore, 100*exstore(:,5)./sxstore(:,5))
xlabel('Time (hr.)')
ylabel(sprintf('Percentage Error Relative to 1\\sigma'), 'Interpreter', 'tex')
title('Ydot')

subplot(2,3,6)
plot(time2hr*txstore, 100*exstore(:,6)./sxstore(:,6))
xlabel('Time (hr.)')
ylabel(sprintf('Percentage Error Relative to 1\\sigma'), 'Interpreter', 'tex')
title('Zdot')

sgtitle('Relative State Estimation Errors');
saveas(gcf, './Simulations/sees_relative.png', 'png');
...
sg = sprintf('Timestamp: %1.5f Hours', tpr*time2hr);
sgtitle(sg)
...
function [Xfit] = stateEstCloud(pf, order_fit, theta_f, R_f, obTr, tdiff, low_lim, up_lim)
    noised_obs = obTr;
    ...
    for i = 1:length(noised_obs(:,1))
        noised_obs(i,3:4) = data_vec(3*(i-1)+2:3*(i-1)+3,1); % AZ-EL Measurements
        noised_obs(i,2) = unifrnd(low_lim, up_lim)/dist2km;
        % while(noised_obs(i,2) < low_lim/dist2km || noised_obs(i,2) > up_lim/dist2km)
        %     noised_obs(i,2) = mvnrnd(partial_ts(i,2), (R_f*partial_ts(i,2))^2);
        % end
